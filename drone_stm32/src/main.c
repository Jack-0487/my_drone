...
{  
  ...
  while(1) {
    if(accgyro.gyroCalibrating || accgyro.accCalibrating) led_ctrl(LED_GYRO_CALIBRATION);  
    else if(fc.flags.motorArmed) led_ctrl(LED_FLIGHT);
    else led_ctrl(LED_IDLE);
...
//    opticalflow_ctrl();
    ...
    if(printTestTimer >= 2) {
      ...
      // 姿态角和气压计高度
      printf("%f,%f,%f,%d,",angle[ROLL]/100.0f, angle[PITCH]/100.0f, angle[YAW]/100.0f, BaroAlt);  
